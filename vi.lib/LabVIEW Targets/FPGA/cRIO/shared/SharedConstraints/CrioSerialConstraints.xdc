#..............................................................................#
#                          Serial Module Slot SLOTNUM                          #
#..............................................................................#
#Constraints for the Serial Modules. These constraints are basically the same as
#the common constraints, however Trigger is used as an async input from the
#module. Also this module doesn't use Convert as a conversion for an ADC, it
#uses both, Func and Conv, as Func lines in order to select which port to
#talk to. Done is not used in these module
...
#Group pins
#   -SpiPins: MOSI, CS_n, Func, Func1(Conv_n)
#   -AsyncPins: Id_Sel, Trig (Spi Pins also go on the async groups
...
set CrioAsyncInputPinsSLOTNUM [get_ports {CRIOPIN_IdSel CRIOPIN_Trig}]
set CrioAsyncOutputPinsSLOTNUM [get_ports {CRIOPIN_IdSel CRIOPIN_Mosi CRIOPIN_Cs_n CRIOPIN_Conv_n CRIOPIN_Func CRIOPIN_SpiClk}]
...
#         SPI Flops          #
#............................#
#Define all the flops for SPI in order to use them for exceptions later on
set CrioSpiOutDriversSLOTNUM [filter -regexp -nocase [all_registers] {(
                              NAME =~ ".*Crio987(0|1)ResourceSLOTNUM.*cRioStock.*SpiEngine\/cOutShiftReg_reg.7.*" ||
                              NAME =~ ".*Crio987(0|1)ResourceSLOTNUM.*cRioStock.*SpiEngine\/cCs_n_reg" ||
                              NAME =~ ".*Crio987(0|1)ResourceSLOTNUM.*cRioStock.*ModeSelector\/cFuncOut_reg" ||
                              NAME =~ ".*Crio9..._?ResourceSLOTNUM.*cRioStock.*cRioStockModeTransx\/cSpiFuncInt_reg" ||
                              NAME =~ ".*Crio987(0|1)ResourceSLOTNUM.*Crio987(0|1)ControlSm.*cSpiFunc0Local_reg" ||
                              NAME =~ ".*Crio987(0|1)ResourceSLOTNUM.*Crio987(0|1)ControlSm.*cSpiFunc1Local_reg"
                               )}]
set CrioNonSpiOutDriversSLOTNUM [filter -regexp -nocase [all_registers] {( NAME =~ ".*Crio987(0|1)ResourceSLOTNUM.*" &&
                                  NAME !~ ".*Crio987(0|1)ResourceSLOTNUM.*cRioStock.*SpiEngine\/cOutShiftReg_reg.7.*" &&
                                  NAME !~ ".*Crio987(0|1)ResourceSLOTNUM.*cRioStock.*SpiEngine\/cCs_n_reg" &&
                                  NAME !~ ".*Crio987(0|1)ResourceSLOTNUM.*cRioStock.*ModeSelector\/cFuncOut_reg" &&
                                  NAME !~ ".*Crio9..._?ResourceSLOTNUM.*cRioStock.*cRioStockModeTransx\/cSpiFuncInt_reg" &&
...
set CrioNonSpiClkDriversSLOTNUM [filter -regexp -nocase [all_registers] {( NAME =~ ".*Crio987(0|1)ResourceSLOTNUM.*" &&
                                  NAME !~ ".*Crio987(0|1)ResourceSLOTNUM.*cRioStock.*SpiEngine\/cSpiClk_reg"
                                )}]
...
create_generated_clock -name CrioSpiClkSLOTNUM \
                       ...
                       -source [get_pins -regexp -nocase ".*Crio987(0|1)ResourceSLOTNUM.*cRioStock.*SpiEngine\/cSpiClk_reg\/Q"] \
                       [get_ports CRIOPIN_SpiClk]

...
#        OUTPUT PINS         #
#............................#
#Get all the SPI Pins Drivers, we are assuming that the Convert Pin is the only
#driver that is not part of criostock or criocommint
...
#..............................................................................#
#                        ClockConditioning  Constraints                        #
#..............................................................................#
#This Constraints are meant for the CrioClockCondition component, for more
#information on this constraints, refer to:
#//microDAQ/Software/ModulesSupport/documentation/Constraints Analysis/C Series IO nodes Constraints Analysis.docx
#
#This constraints run under the assumption that the Resource follows the
#structure Crio9XXX with or without a _  and then resource (For Example:
#Crio9201_Resource and Crio9205Resource are valid, but Crio8838ImmResouce is
#not). It also assumes that the CrioClockCondition.vhd is instantiated and at
#least keeps the CrioClockCondition as part of the instantation name (For
#Example: CrioClockConditionx: entity work.CrioClockCondition (rtl) is ok
#but CleaningCircuit: entity work.CrioClockCondition (rtl) is not ok.

#Set the max delay between the FallFF to the CLR of RiseFF and Q of Delay1FF
set_max_delay -quiet -datapath_only 2.626 \
              -from [get_cells -regexp {.*Crio9..._?ResourceSLOTNUM.*CrioClockCondition.*FallFF.*}]

#Set the max delay between the FallFF Delayed to the CLR of the FallFF
set_max_delay -quiet -datapath_only 2.626 \
              -from [get_cells -regexp {.*Crio9..._?ResourceSLOTNUM.*CrioClockCondition.*Delay2FF.*}]

#OClk is not in a Clock Pin, so Xilinx complains about it, this is a workaround to
#demote the Error and allow us use OClk as a clock
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets -of_objects [all_fanout -flat -only_cells [get_ports CRIOPIN_OClk]]]